...
`twMerge` tries its best to remove classes on the left that should be overriden by classes on the right. But it's solvable without Javascript, because the problem isn't dynamic and doesn't change at runtime.
...
Here's a straightforward version that simply duplicates classes into a lower layer.
...
Here's an interesting alternative using the compilation feature.
