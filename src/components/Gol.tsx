...
function randomize(board: boolean[][]) {
...
function nextBoard(prevBoard: boolean[][]) {
...
function useIsBigScreen() {
  ...
  useEffect(() => {
    ...
    const update = () => setBig(mql.matches);
    update();
  ...
  return (
    <>
      {bigScreen && showControls && (
        <div className="fixed z-10 top-4 left-4 flex flex-col gap-2 bg-stone-50 p-2 border shadow-2xl">
          ...
          <input
            ...
            defaultValue="#fda4af"
            onChange={(e) => {
              color.current = `${e.currentTarget.value}80`;
            }}
          />
      ...
      <canvas
        ...
        width={width}
        height={height}
        className="-z-10 absolute w-screen top-0 left-0 opacity-25 pixelated"
        onMouseDown={(e) => {
