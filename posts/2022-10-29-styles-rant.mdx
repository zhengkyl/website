...
- style code blocks
- include links to utility class articles
- [x] edit pass 1
- [x] edit pass 2
- [ ] edit pass 3
- [ ] edit pass 4
...
- Return from Narnia and find your place
...
It is similar to the inline `styles` prop on regular elements, and it accepts most CSS properties.

```html
<Box sx={{ display: "flex" }}/>
```

For spacing properties, numerical values are multiples of the spacing size defined in the theme.
...
The font related properties take strings that correspond to values defined in the theme, or numerical overrides.

```html
<Box sx={{}}>…</Box>

<Box sx={{ fontWeight: 'light' }}>…</Box>

<Box sx={{ fontWeight: 'regular' }}>…</Box>

<Box sx={{ fontWeight: 'medium' }}>…</Box>
...
Color properties accept strings corresponding to values defined in the theme.

```html
<Box sx={{ color: 'primary.main' }} />
```

`boxShadow` accepts a number corresponding to shadows predefined in the theme.

```html
<Box sx={{ boxShadow: 1 }} />
```

And of course, there are shorthand property names to help avoid typing long property names.

There is `m`, `mt`,`mr`, `mb`, `ml`, `mx`, `my` along with the padding variations and `background` can just be `bg`.

There's also some specific properties and interactions, but I can't really list out the entire functionality of `mui`. Check out their [documentation](https://mui.com/system/getting-started/the-sx-prop/) for more information.

This is very nearly close to perfect.

We have access to all the CSS we already know, and it's a bit more ergonomic with the addition reasonble shorthands.

Properties that should stay consistent use values defined in the theme, and we don't have to mess with actual values or units, unless we deliberately want to.
...
Styles need to be defined with the content they style. This is not an argument. Wherever the notion that content and styling needed to be separate came from, it has been disproven..
