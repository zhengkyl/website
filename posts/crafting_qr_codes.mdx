...

{
  <small className="text-stone-700">
    There are way more Chinese examples, especially with animation and integration into larger designs, but those don't fit here nicely. I recommend using Pinterest to find them.
  </small>
}

Having a human fine-tune each aspect does lead to the best results, and I figured the limits of programmatically generated QR codes had long been reached. Then I saw these two style options on [QRBTF.com](https://qrbtf.com) created by [Troy Ni](https://troyni.com/) and [@CPunisher](https://github.com/CPunisher).
...
It's so obvious in hindsight, but I never even considered breaking the mold this much. As if seeing for the first time, all the world around me turned into inspiration.
...
The end result is https://qrframe.kylezhe.ng, a web app that lets you write code and define parameters to render QR codes however you want. Here's the [source code](https://github.com/zhengkyl/qrframe) for the perverts.
...
This was a failure for many reasons (it takes forever to draw and smears instantly), but the biggest issue was that the end result couldn't be scanned *by the default camera app*.

While in the past we had to use random QR code scanner apps, this feature has long since been handled by OEMs as part of the camera app. Unfortunately, this comes with a tradeoff.

Smartphone camera apps are not dedicated QR code scanners. They don't even attempt to scan until after a QR code is detected using computer vision.
    - In my experience, this favors **solid colors** and **high contrast**, and my chalk didn't contrast enough against or completely cover the gray concrete. This is also likely why some QR codes scan better from farther away.

My second mistake was not at all understanding where I needed precision and where I could cut corners. For my first attempt, I even drew the finder patterns by hand without a guide.

<QrTutorial/>

## Prior work

Basically, as long as the space where a QR code pixel should be is dark enough or light enough, it will be decoded properly.
...

{
  <div className="grid cols-2 font-thin text-xs text-center">
    <div>
      <img className="p-4" src="ai_mike.jpg" alt="ai generated mike wazowski qr code"/>
      AI generated using <Sa href="https://qrbtf.com">QRBTF</Sa>
    </div>
  </div>
}
