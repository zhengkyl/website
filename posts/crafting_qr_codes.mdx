...
Beyond simply placing a logo on top, there have also been many attempts to create "artistic" QR codes. Forming an image is the obvious thing to do, and there are various ways of combining the QR code pixels with an picture for a unique look.
...
It's available at https://qrframe.kylezhe.ng, and it's a tool for crafting QR codes. It comes with presets for my original designs as well as shameless copies of existing artistic QR codes, but the built-in code editor lets you freely create new presets or modify existing ones. It's all just Javascript that generates an SVG or renders to an HTML canvas.
...
So yeah I lied, all you needed to know is that the finder pattern has to exist, and everything else just needs to be dark or light enough.

Unfortunately, there's one major gotcha we've gotta cover.
