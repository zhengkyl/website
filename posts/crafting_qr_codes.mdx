...
While limited in range, these techniques are not specific to a single QR code and generalize to any image and URL. This is in stark contrast to the labor intensive but fully bespoke QR codes made by human designers.
...
I know now two things I failed to account for.

1. Smartphone camera apps are not dedicated QR code scanners. They don't even attempt to scan until after a QR code is detected by a computer vision model.
    - <p className="py-2">In my experience, they favor **solid colors** and **high contrast**, and my chalk didn't contrast enough against or completely cover the gray concrete. This is likely why some QR codes scan better from farther away.</p>

2. The finder patterns have to be somewhat precise for the decoding process to start.
