...
The natural progression impels beating the dead elephant in the room called generative AI. These AI QR codes promise beautiful and customizable designs at scale, and when using carefully engineered prompts the results appear good. But I've no prompting expertise, so I've generated naught but nightmares.
...
To this day, I still haven't seen anyone else make QR codes like this. So I made some myself.
...
The generator is available at https://qrframe.kylezhe.ng, but it's more of a tool for crafting QR codes. It comes with presets that are simple to modify or create from scratch, because it's just Javascript that generates an SVG or renders to an HTML canvas.
...
With that in mind, let's cover existing ways people have successfully created artistic codes.

## Image Techniques

There is a wide breadth and depth of research into creating images with QR codes, so I'll only briefly cover these two made-up categories.
...

{
  <div className="grid cols-1 sm:cols-4 gap-2 pixelated py-2">
    ...
    <p className="sm:col-span-3 pb-8 sm:py-2">
      The most unique technique I've seen involves generating word clouds. In this case, the overlay dots are used in an intermediate step to inform a text deforming algorithm.
    </p>
    <div className="font-thin text-xs text-center">
    ...
    <p className="sm:col-span-3 pb-8 sm:py-2">
      The natural progression to animation is simple, since the same dot overlay can be drawn on each frame.
    </p>
  </div>
}
...

{
  <div className="grid cols-1 sm:cols-4 gap-2 pixelated py-2">
    ...
    <p className="sm:col-span-3 pb-8 sm:py-2">
      This site lets you draw the desired output data, which can be directly interpreted as the input text. The data starts with the url to a link redirect service and the random characters that follow act as a unique key. For details, see <a href="https://marienraat.nl/hacking-qr-codes.html">Hacking QR code design</a>.
    </p>
    <div className="font-thin text-xs text-center">
    ...
    <p className="sm:col-span-3 pb-8 sm:py-2">
      This is similar to the previous technique, except that the character count indicator included in the header is set to exclude everything after the desired URL. In other words, they are changing the padding bytes. There's more, but I don't understand it fully. <a href="https://xyzzy.github.io/2020/08/20/Show-HN-Photo-Realistic-QR-Codes.html">See the HN post</a>.
    </p>
    <div className="font-thin text-xs text-center">
    ...
    <p className="sm:col-span-3 pb-8 sm:py-2">
      In terms of animation, reverse-engineering the data for every "frame" is a bit more tedious, and the QR code is unlikely to be scannable.
    </p>
  </div>
}
...
However, not all that glitters is a scannable QR code. The original design is sick, but the gap between dots is somewhat problematic. Scanning from a distance works, but for my recreation I opted to fill the gaps with a gray shadow. This is admittedly much less cool, but many such compromises are inevitable.
...
While it had nothing to do with this mosaic in particular, exposure to all types of art is probably the best source of creative energy. If there's a lesson to be learned here, it's that rules are made to be respectfully broken. You CAN mess with the finder patterns, but notice how the cross shape is completely preserved in two of them.
...
I hope you learned something interesting, even if there aren't too many useful applications for artistic QR codes.
